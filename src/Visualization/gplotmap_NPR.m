function handle = gplotmap_NPR(A,xy,map,pcolor,ecolor,p_val,best_conductance)
...
f = find(map(i) > map(j));
if length(f) 
    xlabel( ['RCheeger:' num2str(best_conductance) ', parts' int2str(nparts) ', p = ' num2str(p_val)],'visible','on');
    if length(ecolor)
